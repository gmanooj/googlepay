...
export default function SetUpiPinScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();

  const paramBankName = (params.bankName as string) || '';
  const [bankName, setBankName] = useState<string>(paramBankName || 'State Bank of India');
        ...
        if (!paramBankName) {
          ...
          if (storedAccounts) {
            ...
            if (Array.isArray(accounts) && accounts.length > 0) {
              setBankName(accounts[0].bankName);
            }
          }
        }
      } catch (err) {
  ...
  const handleKeyPress = (val: string) => {
    ...
    const setPinFn = step === 'enter' ? setPin : setConfirmPin;

    if (val === 'backspace') {
      setPinFn(prev => prev.slice(0, -1));
    } else {
      if (currentPin.length >= 6) return;
      setPinFn(prev => prev + val);
    }
  };
  ...
  const handleCheckClick = async () => {
    ...
    if (step === 'enter') {
      ...
      setLoading(true);
      try {
        const response = await fetch(`${API_HOST}/api/set-upi-pin`, {
          method: 'POST',
        ...
        if (data.success) {
          ...
          Alert.alert('Error', data.message || 'Failed to register UPI PIN.');
        }
      } catch (err) {
        console.error('Failed to set UPI PIN:', err);
        Alert.alert('Network Error', 'Could not link UPI PIN to server.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const getMaskedAccount = () => {
    const cleanUpi = recipientUpiId ? recipientUpiId.split('@')[0] : '';
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.checkBalanceBar}>
        <Text style={styles.checkBalanceText}>Setup UPI Mode</Text>
      </View>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.pinTitle}>
          {step === 'enter' ? 'SET 6-DIGIT UPI PIN' : 'CONFIRM 6-DIGIT UPI PIN'}
        </Text>

        {/* 6 Lines Spaced Pin Input mask */}
        <View style={styles.linesRow}>
          {[0, 1, 2, 3, 4, 5].map(idx => {
            const hasDigit = currentPin.length > idx;
            return (
        ...
        <View style={styles.alertWarningBox}>
          ...
          <Text style={styles.alertText}>
            Do not share this PIN with anyone. You will enter this PIN to authorize future payments.
          </Text>
        </View>
      ...
      <View style={styles.keypadWrapper}>
        ...
        <View style={styles.keypadRow}>
          ...
          <TouchableOpacity 
            style={[styles.checkmarkBtn, currentPin.length < 6 && styles.disabledCheckBtn]} 
